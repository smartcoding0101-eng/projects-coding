...
export default function Configuracion({ settings, auth }) {
    ...
    const ToggleSwitch = ({ settingKey, labelOn = "ACTIVO", labelOff = "INACTIVO" }) => {
        ...
        return (
            <div className="flex bg-fapclas-100 p-0.5 rounded border border-brand display-inline-flex w-max">
                <button type="button" onClick={() => handleChange(settingKey, 'si')}
                    className={`px-3 py-1 rounded-sm text-[10px] uppercase font-bold tracking-wider transition-colors min-w-[80px] ${isOn ? 'bg-card-fap text-brand-main shadow-sm border border-brand' : 'text-brand-muted hover:text-primary'}`}>
                    {labelOn}
                </button>
                <button type="button" onClick={() => handleChange(settingKey, 'no')}
                    className={`px-3 py-1 rounded-sm text-[10px] uppercase font-bold tracking-wider transition-colors min-w-[80px] ${!isOn ? 'bg-red-50 text-red-700 shadow-sm border border-red-500/50' : 'text-brand-muted hover:text-red-500'}`}>
                    {labelOff}
                </button>
            </div>
        );
    ...
    const SectionHeader = ({ icon: Icon, title, subtitle }) => (
        <div className="px-5 py-3 border-b border-brand bg-card-fap flex items-center justify-between">
            <div className="flex items-center gap-3">
                <Icon className="w-4 h-4 text-primary" />
                <div>
                    <h3 className="text-xs font-bold text-brand-main uppercase tracking-wide">
                        {title}
                    </h3>
                    {subtitle && <p className="text-[10px] text-brand-muted font-medium">{subtitle}</p>}
                </div>
            </div>
        </div>
    );
    ...
    const SettingRow = ({ title, description, children, border = true }) => (
        <div className={`flex flex-col md:flex-row md:items-center justify-between py-4 ${border ? 'border-t border-brand' : ''} gap-4`}>
            <div className="flex-1 pr-4">
                <h4 className="font-bold text-brand-main text-[11px] uppercase tracking-wider">{title}</h4>
    ...
    return (
        <AuthenticatedLayout header={<h2 className="font-bold text-xl text-brand-main leading-tight">Módulo E-commerce / Configuraciones</h2>}>
            ...
            <div className="py-8 bg-main min-h-screen">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
                    ...
                    <div className="bg-card-fap border border-brand shadow-sm rounded-lg overflow-hidden flex flex-col md:flex-row md:items-center justify-between p-4 bg-card-fap gap-4 sticky top-0 z-10">
                        <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                onClick={submit}
                                disabled={processing}
                                className="px-5 py-2.5 bg-primary hover:bg-primary-dark text-white text-xs font-bold flex items-center gap-2 shadow-sm transition-colors active:translate-y-px disabled:opacity-50"
                            >
                    ...
                    <form onSubmit={submit} className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="bg-card-fap rounded-lg shadow-sm border border-brand overflow-hidden">
                            ...
                            <div className="bg-card-fap rounded-lg shadow-sm border border-brand overflow-hidden">
                                ...
                                <div className="px-5 py-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 py-4 border-t border-brand">
                                        <div>
                                            <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Dcto. Global Socios (%)</label>
                                            <input 
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Límite Crédito (Bs.)</label>
                                            <input 
                                                type="number" 
                            ...
                            <div className="bg-card-fap rounded-lg shadow-sm border border-brand overflow-hidden">
                                ...
                                <div className="px-5 py-5">
                                    <div className="flex gap-4 items-center">
                                        <div className="w-24 h-24 bg-brand/10 border border-dashed border-brand rounded mx-auto sm:mx-0 flex items-center justify-center overflow-hidden flex-shrink-0">
                                            {qrUrl ? (
                                                <img src={qrUrl.startsWith('/storage') || qrUrl.startsWith('http') ? qrUrl : `/storage/${qrUrl}`} className="w-full h-full object-contain" />
                                            ) : (
                                                <span className="text-[9px] font-bold text-brand-muted uppercase tracking-wider text-center px-2">SIN IMAGEN</span>
                                            )}
                                        </div>
                                        <div className="flex-1">
                                            <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Matriz QR de Cobro</label>
                                            <input 
                                                type="file" 
                                                onChange={e => setData('qr_file', e.target.files[0])}
                                                className="block w-full text-xs text-brand-muted
                                                    file:mr-4 file:py-1 file:px-3 file:border-0
                                                    file:text-[10px] file:font-bold file:uppercase file:tracking-wider
                                                    file:bg-fapclas-100 file:text-primary
                                                    hover:file:bg-fapclas-200 transition-colors cursor-pointer border border-brand rounded p-1" 
                                                accept="image/*" 
                                            />
                                            <p className="text-[10px] text-brand-muted mt-2 leading-tight">Carga la imagen JPG/PNG del código QR de la cuenta recaudadora institucional.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="bg-card-fap rounded-lg shadow-sm border border-brand overflow-hidden">
                                ...
                                <div className="px-5 py-4 space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Cintillo / Badge</label>
                                            <input 
                                                type="text" 
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Frase Subtítulo</label>
                                            <input 
                                                type="text" 
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Título Jerarquía H1</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Cuerpo de Texto (Descripción)</label>
                                        <textarea 
                            ...
                            <div className="bg-card-fap rounded-lg shadow-sm border border-brand overflow-hidden">
                                ...
                                <div className="px-5 py-4 grid grid-cols-2 gap-x-4 gap-y-4">
                                    <div className="col-span-2">
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Dirección Oficial de Bodega</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div className="col-span-2">
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Horario Laboral</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Línea Fija</label>
                                        <input 
                                            type="text" 
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-bold text-primary uppercase tracking-wider mb-1">Enlace WhatsApp</label>
                                        <input 
                                            type="text" 
                            ...
                            <div className="bg-red-50 border border-red-500/50 rounded-lg shadow-sm overflow-hidden">
                                <div className="px-5 py-3 border-b border-red-500/50 bg-red-100/50 flex items-center gap-3">
                                ...
                                <div className="px-5 py-4">
                                    <SettingRow title="MODO MANTENIMIENTO (APAGADO DE TIENDA)" description="Al activar esto, el e-commerce será inaccesible al público." border={false}>
                                        <div className="flex bg-card-fap p-0.5 rounded border border-red-500/50 display-inline-flex w-max">
                                            <button type="button" onClick={() => handleChange('ecommerce_modo_mantenimiento', 'si')}
                                                className={`px-3 py-1 rounded-sm text-[10px] uppercase font-bold tracking-wider transition-colors min-w-[80px] ${data.settings['ecommerce_modo_mantenimiento'] === 'si' ? 'bg-red-600 text-white shadow-sm' : 'text-brand-muted hover:text-red-600'}`}>
                                                ESTADO OFFLINE
                                            </button>
                                            <button type="button" onClick={() => handleChange('ecommerce_modo_mantenimiento', 'no')}
                                                className={`px-3 py-1 rounded-sm text-[10px] uppercase font-bold tracking-wider transition-colors min-w-[80px] ${data.settings['ecommerce_modo_mantenimiento'] !== 'si' ? 'bg-primary text-white shadow-sm' : 'text-brand-muted hover:text-brand-main'}`}>
                                                ESTADO ONLINE
                                            </button>
                                        </div>
                                    </SettingRow>
                                    <div className="mt-4 pt-4 border-t border-red-500/50/50">
                                        ...
                                        <textarea 
                                            ...
                                            rows={2} 
                                            className="field-input text-[11px] w-full border-red-500/50 focus:border-red-500 focus:ring-red-500 bg-card-fap" 
                                        />
                                    </div>
                                </div>
                            </div>
