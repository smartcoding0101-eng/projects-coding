...
export default function EstadoCuenta({ auth, fecha_generacion, socio, resumen, creditos, movimientos, socios }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-brand-main leading-tight">Estado de Cuenta</h2>}
        >
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    {isAdmin && socios.length > 0 && (
                        <div className="bg-card-fap shadow-sm rounded-lg p-4 flex items-end space-x-3">
                            <div className="flex-1">
                                <label className="block text-xs text-brand-muted mb-1">Seleccionar Socio</label>
                                <select value={socioId} onChange={(e) => setSocioId(e.target.value)} className="w-full text-sm border-brand rounded-md">
                    ...
                    {/* Info del socio */}
                    <div className="bg-card-fap shadow-sm rounded-lg p-5">
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                            <div><span className="text-brand-muted">Nombre:</span><p className="font-bold">{socio.nombre}</p></div>
                            <div><span className="text-brand-muted">CI:</span><p className="font-bold">{socio.ci}</p></div>
                            <div><span className="text-brand-muted">Grado:</span><p className="font-bold">{socio.grado}</p></div>
                            <div><span className="text-brand-muted">Destino:</span><p className="font-bold">{socio.destino}</p></div>
                            <div><span className="text-brand-muted">Escalafón:</span><p className="font-bold">{socio.escalafon}</p></div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-card-fap rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-brand-muted uppercase">Saldo Kardex</p>
                            <p className="text-2xl font-bold text-primary">Bs. {Number(resumen.saldo_kardex).toFixed(2)}</p>
                        </div>
                        <div className="bg-card-fap rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-brand-muted uppercase">Créditos Activos</p>
                            <p className="text-2xl font-bold text-blue-600">{resumen.creditos_activos}</p>
                        </div>
                        <div className="bg-card-fap rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-brand-muted uppercase">Deuda Total</p>
                            <p className="text-2xl font-bold text-red-600">Bs. {Number(resumen.deuda_total).toFixed(2)}</p>
                        </div>
                        <div className="bg-card-fap rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-brand-muted uppercase">Total Pagado</p>
                    ...
                    {/* Créditos */}
                    {creditos.length > 0 && (
                        <div className="bg-card-fap shadow-sm rounded-lg overflow-hidden">
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-brand/5">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left text-xs font-medium text-brand-muted uppercase">Pagadas/Pendientes</th>
                                    </tr>
                                </thead>
                    ...
                    {movimientos.length > 0 && (
                        <div className="bg-card-fap shadow-sm rounded-lg overflow-hidden">
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-brand/5">
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-brand-muted uppercase">Fecha</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-brand-muted uppercase">Tipo</th>
                                        <th className="px-4 py-3 text-left text-xs font-medium text-brand-muted uppercase">Concepto</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-brand-muted uppercase">Ingreso</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-brand-muted uppercase">Egreso</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-brand-muted uppercase">Saldo</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200">
