...
export default function Morosidad({ auth, fecha_generacion, resumen, cuotas }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Reporte de Morosidad</h2>}
        >
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-white rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-gray-500 uppercase">Cuotas Atrasadas</p>
                            <p className="text-2xl font-bold text-red-600">{resumen.total_cuotas_atrasadas}</p>
                        </div>
                        <div className="bg-white rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-gray-500 uppercase">Socios Afectados</p>
                            <p className="text-2xl font-bold text-orange-600">{resumen.socios_afectados}</p>
                        </div>
                        <div className="bg-white rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-gray-500 uppercase">Capital Moroso</p>
                            <p className="text-2xl font-bold text-red-700">Bs. {Number(resumen.total_capital_moroso).toFixed(2)}</p>
                        </div>
                        <div className="bg-white rounded-lg shadow-sm border p-5">
                            <p className="text-xs text-gray-500 uppercase">Mora Acumulada</p>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {cuotas.length === 0 && (
                                        <tr><td colSpan={9} className="px-6 py-8 text-center text-gray-400">✅ Sin morosidad registrada</td></tr>
                                    )}
                                    {cuotas.map((c, i) => (
                                        <tr key={i} className="hover:bg-red-50">
                                            ...
                                            <td className="px-3 py-3 text-sm">
                                                ...
                                                <div className="text-xs text-gray-400">{c.ci} · {c.grado}</div>
                                            </td>
                                            <td className="px-3 py-3 text-sm text-gray-600">{c.tipo_credito}</td>
                                            <td className="px-3 py-3 text-sm">{c.nro_cuota}</td>
                                            <td className="px-3 py-3 text-sm text-gray-500">{c.fecha_vencimiento}</td>
