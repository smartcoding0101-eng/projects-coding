                    ...
                    {items.map((item, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="text-gray-100 italic mb-10 font-sans leading-relaxed text-lg min-h-[120px]">"{item.content}"</p>

                                <div className="flex items-center gap-4 relative z-10 border-t border-white/10 pt-6">
                                    {item.image ? (
                                        <img src={getImageUrl(item.image)} alt={item.name} className="w-14 h-14 rounded-full border-2 border-secondary shadow-lg object-cover" />
                                    ) : (
                                        <div className="w-14 h-14 rounded-full border-2 border-secondary bg-white/10 flex items-center justify-center text-white font-bold text-xl shadow-lg">
                                            {item.name?.charAt(0)}
                                        </div>
                                    )}
                                    <div>
                                        <h4 className="font-bold text-white text-lg tracking-tight">{item.name}</h4>
                                        {item.role && <p className="text-xs text-secondary tracking-widest uppercase font-bold mt-1">{item.role}</p>}
                                    </div>
                                </div>
                            </div>
                        );
