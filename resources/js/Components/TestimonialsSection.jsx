...
const defaultTestimonials = [
    {
        name: "Sgto. Jorge Luis",
        role: "La Paz, Bolivia",
        content: "Gracias a los créditos rápidos de Fapclas pude renovar mi hogar sin trámites burocráticos. La atención fue inmediata, el mismo día desembolsaron mi crédito.",
        rating: 5,
        image: "https://images.unsplash.com/photo-1599566150163-29194dcaad36?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&q=80",
    },
    {
        name: "Sof. Carmen Rosa",
        role: "Santa Cruz",
        content: "La caja de ahorro me brinda una rentabilidad del 5% y la comodidad de consultar mi capital desde el celular. Totalmente recomendado para nuestra familia policial.",
        rating: 5,
        image: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&q=80",
    },
    {
        name: "Sgto. My. Alberto C.",
        role: "Cochabamba",
        content: "Es un alivio contar con convenios con las empresas que usamos a diario. Sacar productos a cuotas por planillas desde la plataforma me parece algo espectacular.",
        rating: 5,
...
function StarRating({ count = 5 }) {
    const stars = Math.min(5, Math.max(1, parseInt(count) || 5));
    return (
...
export default function TestimonialsSection({ cmsData = null }) {
    // CMS tiene prioridad — fallback a valores hardcodeados
    const title = cmsData?.title || 'Voces de Nuestra Familia';
    const subtitle = cmsData?.subtitle || 'Confianza que nos respalda';

    const items = (cmsData?.items && cmsData.items.length > 0)
        ? cmsData.items
        : defaultTestimonials;
        ...
        <section className="py-12 bg-primary text-white relative overflow-hidden" id="testimonios">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="text-center mb-8">
                    <h2 className="text-sm font-bold tracking-widest text-secondary uppercase mb-3 drop-shadow-sm">{subtitle}</h2>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {items.slice(0, 3).map((item, index) => (
                        ...
                        >
                            ...
                            <p className="text-gray-100 italic mb-10 font-sans leading-relaxed text-lg min-h-[120px]">
                                "{item.content}"
                            </p>
                            ...
                            <div className="flex items-center gap-4 relative z-10 border-t border-white/10 pt-6">
                                {(item.image || item.avatar) && (
                                    <img
                                        src={getImageUrl(item.image || item.avatar)}
                                        alt={item.name}
                                        className="w-14 h-14 rounded-full border-2 border-secondary shadow-lg object-cover"
                                    />
                                )}
                                <div>
                                    <h4 className="font-bold text-white text-lg tracking-tight">{item.name}</h4>
                                    <p className="text-xs text-[#F7BD16] tracking-widest uppercase font-bold mt-1">{item.role}</p>
                                </div>
                            </div>
                        </div>
                    ))}
