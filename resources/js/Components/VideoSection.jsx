...
export default function VideoSection({ cmsData = null }) {
    ...
    const sectionTitle = cmsData?.title || 'Fapclas en Movimiento';
    const sectionSubtitle = cmsData?.subtitle || 'Conoce Nuestra Historia';

    const localVideoFile = cmsData?.local_video_file ? getUrl(cmsData.local_video_file) : null;
    const mainVideoUrl = localVideoFile || cmsData?.main_url || 'https://vjs.zencdn.net/v/oceans.mp4';
    ...
    const galleryVideos = (cmsData?.gallery && cmsData.gallery.length > 0)
        ...
        <section className="py-12 bg-surface-container overflow-hidden relative" id="videos">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row gap-8 items-end justify-between mb-6 relative z-10">
                    <div>
                        <h2 className="text-sm font-bold tracking-widest text-primary uppercase mb-3">{sectionSubtitle}</h2>
                        <h3 className="font-display text-4xl font-bold text-on-surface">{sectionTitle}</h3>
                    </div>
