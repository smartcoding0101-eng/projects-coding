...
const defaultBenefits = [
    {
        ...
        description: "Aportes protegidos por activos reales y auditorías constantes.",
        icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>,
    },
    {
        title: "Tienda Digital",
        description: "Crédito inmediato en tecnología y hogar por planilla.",
    ...
    },
    {
        ...
        description: "Salud, lavandería y bordados con tarifas preferenciales.",
        icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>,
    },
    {
        title: "Retorno Social",
        description: "Distribución anual de excedentes por antigüedad.",
        icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
    },
];

export default function BenefitsSection({ cmsData = null }) {
    // CMS tiene prioridad — fallback a valores hardcodeados
    const statusLabel = cmsData?.status_label || 'Socio Activo';
    const statusBadge = cmsData?.status_badge || 'Fapclas ERP v3';

    const benefits = (cmsData?.items && cmsData.items.length > 0)
        ? cmsData.items.map(item => ({
            title: item.title || '',
            description: item.description || item.desc || '',
            link: item.link || null,
            icon: null, // íconos de Filament son strings (nombre Lucide), no JSX; usamos un icono genérico
        }))
        : defaultBenefits;
                ...
                <div className="mt-2 bg-primary-dark rounded-2xl p-5 flex items-center justify-between relative overflow-hidden text-white">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold font-display uppercase tracking-widest">{statusLabel}</span>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-[10px] font-black text-secondary uppercase">{statusBadge}</div>
